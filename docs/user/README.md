...
| `spec.RemoteURL`               | RemoteURL is the remote registry URL. The format must be `<scheme><host>[:<port>]` where `<scheme>` is `https://` or `http://` and `<host>[:<port>]` corresponds to the Upstream. <br> If defined, the value is set as `proxy.remoteurl` in the registry [configuration](https://github.com/distribution/distribution/blob/main/docs/content/recipes/mirror.md#configure-the-cache); and in containerd configuration as `server` field in [hosts.toml](https://github.com/containerd/containerd/blob/main/docs/hosts.md#server-field) file. | N/A           | No       |
...
| `spec.garbageCollection.ttl`   | The time-to-live (TTL) duration for cached images. Images that have not been accessed within this duration will be eligible for garbage collection. The duration must be specified in a format recognized by Go's `time.ParseDuration` function (e.g., "24h" for 24 hours). Set to 0s to disable the garbage collection.                                                                                                                                                                                                                    | 168h (7 days) | No       |
