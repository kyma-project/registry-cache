...
If the CR is accepted, KCP processes it. The status transitions from `Pending` to `Ready` on success, or to `Error` if processing on the KCP side fails. For details, check `status.conditions`.
...
| **spec.garbageCollection.ttl**   | Must be in a format recognized by Go's `time.ParseDuration` (for example, `24h`). Set to `0s` to disable garbage collection. If disabled, you cannot re-enable it.        |
